...
def parse_date(date_str):
    ...
    try:
        return date_parser.parse(
            date_str, default=datetime(datetime.now().year, 1, 1)
        )
    except Exception:
        return None
...
DATE_RANGE_REGEX = re.compile(
    rf"""
    (?P<start>(?:{MONTHS}\s+\d{{4}}|\d{{4}}))
    \s*[-–to]+\s*
    (?P<end>(?:{MONTHS}\s+\d{{4}}|\d{{4}}|present|current|now))
    """,
...
def calculate_component_scores(
    ...
    for i, resume_raw in enumerate(resumes_raw):
        ...
        final_score = raw_score / sum(weights.values())
...
def calculate_ats_score(resume_text, job_keywords=None):
    ...
    skill_score = (
        len(resume_skills & {k.lower() for k in job_keywords}) / max(1, len(job_keywords))
        if job_keywords else min(1.0, len(resume_skills) / 8.0)
    )

    section_score = sum(
        s in text for s in ["experience", "education", "skills", "projects", "summary"]
    ) / 5
    ...
    return round(min(1.0, final_score) * 100, 2)
